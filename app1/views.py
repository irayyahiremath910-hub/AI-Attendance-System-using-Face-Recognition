...
def dashboard(request):
    ...
    from .utils import get_dashboard_statistics
    
    days = int(request.GET.get('days', 30))
    stats = get_dashboard_statistics(days)
    
    context = {
        'stats': stats,
        'selected_days': days,
        'days_choices': [7, 15, 30, 60, 90]
    }
...
def attendance_report(request):
    ...
    from .utils import get_student_report
    ...
    if student_id:
        report = get_student_report(student_id, int(days))
...
def export_attendance_csv(request):
    ...
    from .utils import export_attendance_to_csv
    
    start_date = request.GET.get('start_date')
    end_date = request.GET.get('end_date')
    student_id = request.GET.get('student_id')
    
    return export_attendance_to_csv(request, start_date, end_date, student_id)
...
def export_attendance_pdf(request):
    ...
    from .utils import export_attendance_to_pdf
    
    start_date = request.GET.get('start_date')
    end_date = request.GET.get('end_date')
    student_id = request.GET.get('student_id')
    
    return export_attendance_to_pdf(start_date, end_date, student_id)
...
def low_attendance_students(request):
    ...
    return render(request, 'low_attendance_students.html', context)
